...
function createNotifierWindow(data) {
    ...
    notifierWindow = new BrowserWindow({
        ...
        height: 130,
        parent: mainWindow,
        x: externalDisplay.width - 320,
        y: externalDisplay.height - 130,
        frame: false,
        resizable: false,
        icon: path.join(__dirname, 'assets/icons/png/icon.png')
    });
        ...
        await page.property('content').then(function() {
            ...
            async function mailWatchFn() {
                ...
                await page.property('content').then(function(content) {
                    ...
                    if (newCount === 0) {
                        mainWindow.tray.setImage(path.join(__dirname, 'assets/ico_tary.png'));
                        mainWindow.tray.setToolTip(`새로운 메일이 없습니다.`);
                    } else {
                        mainWindow.tray.setImage(path.join(__dirname, 'assets/ico_tary_n.png'));
